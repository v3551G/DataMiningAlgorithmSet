...
/**
 * 非叶子节点
 * // 非叶子节点的孩子节点可能为非叶子节点，也可能为叶子节点；如果是叶子节点的孩子，则以双向链表的形式存在，
 *    即二者肯定有一个为空
 * @author lyq
 * 
 */
public class NonLeafNode extends ClusteringFeature {
	...
	private NonLeafNode parentNode; // 父亲节点
    // 
	...
	public boolean addingNeededDivide(LeafNode leafNode) {
		...
		if (leafChilds == null) {
			leafChilds = new LinkedList<LeafNode>();
	...
	public LeafNode findedClosestNode(Cluster cluster) {
        ...
        // 如果下一层是叶节点，则一次就可以；如果下一层是非叶节点，则递归遍历直至找到据其最近的叶子节点
		if (nonLeafChilds == null) {
	...
	protected void directAddCluster(ClusteringFeature node) {
		...
		if (parentNode != null) {
			parentNode.directAddCluster(node);   // 递归方式， 自当前层层往上执行
		}
